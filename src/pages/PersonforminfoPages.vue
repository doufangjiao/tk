<template>
	<div>
		<personform-header></personform-header>
		<personform-main></personform-main>
		<personform-job></personform-job>
	</div>
</template>

<script>
import '../assets/css/personforminfo.css'
import '../assets/js/jquery.min.js'
import '../assets/js/personforminfo.js'

import PersonformHeader from '../components/personforminfo/PersonformHeader.vue'
import PersonformMain from '../components/personforminfo/PersonformMain.vue'
import PersonformJob from '../components/personforminfo/PersonformJob.vue'

export default {

  name: 'PersonforminfoPages',
 ...
 mounted:function () {
		...
		$('.job').click(function(event){
			event.stopPropagation();
			$('.personforminfo-cover').css('display','block');
			$('.father-bar').stop(true).animate({
				'opacity':'1',
				'right':'0rem'
			},400);
		});
		$('.close-type,.son-son-bar>li').click(function(event){
			event.stopPropagation();
			$('.father-bar').stop(true).animate({
				'right':'-4.48rem',
				'opacity':'0'
			},400,function(){
				$('.personforminfo-cover').css('display','none');
				$('.father-bar').css({
					'display':'block',
					'zIndex':'30',
					'right':'0',
		...
		//职业选项
		$('.father-bar>li').click(function(event){
			event.stopPropagation();
			...
			$('.selected>.one').html($(this).children('span').html());
		});
		$('.son-bar>li').click(function(event){
			event.stopPropagation();
			$(this).children('ul').css({
				'display':'block',
				'zIndex':'1',
			...
			$('.selected>.two').html($(this).children('span').html());
		});
		$('.son-son-bar>li').click(function(){
			let jobtype1=$('.selected>.one').html();
			let jobtype2=$('.selected>.two').html();
			let jobtype3=$(this).children('span').html();
			let jobtype123= jobtype1+'-'+jobtype2+'-'+jobtype3;
			$('.form-content>.job>input').val(jobtype123);
		//	$('.not-mine-form>.job>input').val(jobtype123);
		});
		...
		$('.insRelation-tit>.sel-who>.lover,.insRelation-tit>.sel-who>.parent').click(function(){
			$('.insRelation>.not-mine-form').css('display','block');
		});
		$('.insRelation-tit>.sel-who>.me').click(function(){
			$('.insRelation>.not-mine-form').css('display','none');
		});

		//阅读声明 
		$('.statement>i').click(function(){
      $(this).removeClass('.read');
			$(this).toggleClass('no-read');
			$('.buyit>button').toggleClass('disable');
      $('.buyit>button').toggleClass('checked');
